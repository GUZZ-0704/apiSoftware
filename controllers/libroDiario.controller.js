...
exports.crearLibroDiario = async (req, res) => {
    try {
        ...
        res.status(500).send({ message: error.message || "Error al crear el libro diario." });
    }
};
...
exports.listaLibrosDiarios = async (req, res) => {
    try {
        ...
        res.status(500).send({ message: error.message || "Error al obtener los libros diarios." });
    }
};
...
exports.listaLibroDiarioPorID = async (req, res) => {
    ...
    try {
        ...
        if (!data) return res.status(404).send({ message: "Libro Diario no encontrado." });
        res.send(data);
    } catch (error) {
        res.status(500).send({ message: error.message || "Error al obtener el libro diario." });
    }
};
...
exports.editarLibroDiario = async (req, res) => {
    ...
    try {
        ...
        res.status(500).send({ message: error.message || "Error al actualizar el libro diario." });
    }
};
...
exports.eliminarLibroDiario = async (req, res) => {
    ...
    try {
        ...
        res.status(500).send({ message: error.message || "Error al eliminar el libro diario." });
    }
};
